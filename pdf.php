...
//INSTÂNCIA DE OPTIONS DO DOMPDF
$options = new Options();                   //ABRE AS OPTIONS
$options->setChroot(__DIR__);               //DEFINE O DIRETÓRIO RAIZ DO PROJETO
$options->setIsRemoteEnabled(true);         //PERMITE LINKS DE FORA NO CORPO DO PDF

//CONSULTA DO ULTIMO VALOR SETADO
$query = "SELECT InputRequerimento FROM requerimentos ORDER BY InputRequerimento DESC LIMIT 1";
$stmt = $pdo->prepare($query);
$stmt->execute();
$result = $stmt->fetch(PDO::FETCH_ASSOC);

//SE A SAÍDA DO BANCO FOR POSITIVA ELE ADICIONA O ID
if ($result) {
    $idRequerimento = $result['InputRequerimento'];
    //echo "Último ID do requerimento: " . $idRequerimento; //DEBUG
} else {
    echo "Nenhum requerimento encontrado."; //DEBUG
}
...
//ATRIBUIÇÃO DO CORPO POR CONCATENAÇÃO
while ($linha = $sql->fetch(PDO::FETCH_ASSOC)) {
    $html = '<h1 style = "margin-top: -6%; margin-bottom: 0%;">SRE-------------------------------------- PRODEN/CGA</h1>';
...
//CONVERTE O HTML NO CORPO
$dompdf->loadHtml($html);

//DEFINE ORIENTAÇÃO (PORTRAIT E LANDSCAPE)
$dompdf-> setPaper('A4', 'portrait');

//RENDERIZA O ARQUIVO
$dompdf->render();

//GERA O NOME DO ARQUIVO COM BASE NO ID DO REQUERIMENTO CHAMADO DO BANCO
$nomeArquivo = 'Requerimento_' .$idRequerimento. '.pdf';

//dALVA O ARQUIVO CRIADO PELO DOMPDF NO CAMINHO ESPECÍFICO
file_put_contents(__DIR__.'/Attachments/'. $nomeArquivo, $dompdf->output());

//DEBUG
//echo 'Arquivo salvo na pasta com o nome: ' . $nomeArquivo;
